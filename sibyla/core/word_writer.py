...
def _resolver_alinhamento(dado: dict, alinhamento: str) -> WD_ALIGN_PARAGRAPH | None:
    """Devolve o alinhamento a aplicar ao parágrafo."""
    if alinhamento != "original":
        return _ALIGN_MAP.get(alinhamento)
    # lógica original: centraliza títulos grandes com recuo pequeno
    font_size = max(8, min(round(dado["tamanho"]), 36))
    if dado["x0"] <= 60 and font_size >= 14 and len(dado["texto"]) < 80:
...
def adicionar_pagina_no_doc(doc, num_pagina: int, blocos_traduzidos: list,
    ...
    """
    # Marcador de página invisível — usado como âncora para replace/append
    p = doc.add_paragraph()
    ...
    for tipo, _, dado in itens:
        if tipo == "imagem":
            ...
            if not texto:
                ...
                continue
            para = doc.add_paragraph()
            font_size = tamanho_fonte if tamanho_fonte else max(8, min(round(dado["tamanho"]), 36))
            if dado["x0"] > 60 and alinhamento == "original":
                para.paragraph_format.left_indent = Pt(dado["x0"] * 0.4)
            align = _resolver_alinhamento(dado, alinhamento)
